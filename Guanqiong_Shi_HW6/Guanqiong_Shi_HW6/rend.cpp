...
*/ 
	...
	for(j=0;j<render->display->yres;j++)
	{
		...
		{
			render->display->fbuf[j*render->display->xres+i].red=2500;
			render->display->fbuf[j*render->display->xres+i].green=970;
			render->display->fbuf[j*render->display->xres+i].blue=0;
			render->display->fbuf[j*render->display->xres+i].alpha=1;
...
*/ 
	...
	if(z01>0&&z02>0&&z03>0)
	{
		...
		x1 += render->AAOffset_X; y1 += render->AAOffset_Y;
		x2 += render->AAOffset_X; y2 += render->AAOffset_Y;
		x3 += render->AAOffset_X; y3 += render->AAOffset_Y;
		...
		// Calculate plane of screen space u, v -- for perspective-correction
		float Au, Bu, Cu, Du;
		float Av, Bv, Cv, Dv;
		...
		//// Calculate plane of image space u, v -- for non-perspective-correction
		...
		*/
		...
		if(hori_edge==0)	// No horizontal edges
		{
						...
						if(xScan>=0&&xScan<=render->display->xres&&yScan>=0&&yScan<=render->display->yres)
						{
							...
							if(zScan_new<=(float)zScan_old)		// If the new Z value is not bigger than the old one, overwrite the pixel
							{
								if(render->tex_fun)
								{
									...
									//// For non-Prospective-Correction
									//us0 = ((-Au0*xScan-Bu0*yScan-Du0)/Cu0);
									//vs0 = ((-Av0*xScan-Bv0*yScan-Dv0)/Cv0);

									//render->tex_fun(us0, vs0, kdkd);
									//render->tex_fun(us0, vs0, kaka);
								...
								else
								{
									...
									{
										introNorm(normX, normY, normZ, xScan, yScan, normCurrent);
										combinedColor(render, normCurrent, render->Kd, render->Ka, render->Ks, colorCurrent);
									}
									else
									{
										// colorCurrent = render->flatcolor
			...
			else if(xInter<x2)		// Middle point 2 is on the right side of the long edge. Then the left point of scan line should be on edge 13, the right point of the scan line should be on edge 12 then edge 23
			{
				yScan=int(y1);
