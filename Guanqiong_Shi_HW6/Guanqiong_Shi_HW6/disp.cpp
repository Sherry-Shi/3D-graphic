...
/* set everything to some default values - start a new frame */
	...
	for(j=0;j<display->yres;j++)
	{
		...
		{
			display->fbuf[j*display->xres+i].red=2500;
			display->fbuf[j*display->xres+i].green=970;
			display->fbuf[j*display->xres+i].blue=0;
			display->fbuf[j*display->xres+i].alpha=1;
